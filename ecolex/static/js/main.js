...
$(document).ready(function () {
    ...
    function init_all() {
      ...
      // Multiselect
      $('.search-form select[multiple]').multiselect({
          buttonClass: '',
      ...
      $('#id_export_by').on('change', function() {
        if ($(this).val() == 'id') {
          $('#id_doc_type').hide();
          $('.export-id').show();
        }
      ...
      $('.export-id').on('click', function() {
          var select = $('#id_doc_id');
          var doc_id = $(this).val();
          ...
          if ($(this).is(':checked')) {
            ...
            select.find('option[value=' + doc_id + ']').remove();
          }
      });
    }
